...
class feature {
public:
	// feature() : len, weight => initialize len and weight
	// => length = len;
	//    weight = alloc(len); => float weight[len];
	// the only way to change the const variable
...
public:
	...
	// friend: could access private and protected member
...
protected:
	static float* alloc(size_t num) {
		...
		try {
			total += num;
			// fail to alloc the request storage space
		...
		// same as NULL, but it is "literal"(could auto change the data type)
		return nullptr;
	}
	size_t length;
	float* weight;
...
public:
	// initialize feature and iso_last
	// float weight[8*p.size()];
	// iso_last = iso;
	pattern(const std::vector<int>& p, int iso = 8) : feature(1 << (p.size() * 4)), iso_last(iso) {
		...
		// pattern1,pattern2 are isomorphic
		// board => rotate => pattern1 == board => pattern2
		...
		for (int i = 0; i < 8; i++) {
			...
			idx.rotate(i);
			for (int t : p) {
				// add 6-tuple pattern to isomorphic[i]
...
public:
	 ...
	 * estimate the value of a given board
	 */
	// V(s) = f1(s) + f2(s) + f3(s) + f4(s) + f5(s) + f6(s) + f7(s) + f8(s)
	// fi(s): weight of isomorphic pattern i
	// same as learning.estimate()
...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
		// TODO
		// index: 0b000000000000000000000000
		//          [ 1][ 2][ 3][ 4][ 5][ 6]
		size_t index = 0;
		for (size_t i = 0; i < patt.size(); i++){
			// i=0:
			// index: 0b000000000000000000000000
			//  or    0b00000000000000000000pat1
			// i=1:
			// index: 0b0000000000000000pat10000
			//  or    0b00000000000000000000pat2
			// i=5:
			// index: 0bpat1pat2pat3pat4pat50000
			//  or    0b00000000000000000000pat6
			index |= b.at(patt[i]) << (4 * i);
		}
		//index: 0bpat1pat2pat3pat4pat5pat6
		return index;
	}
...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				// space: board
				// s --a--> s' --pop--> s''
				// EVALUATE(s,a) = r + sum[P(s,a,s'')*V(s'')]
				// P(s,a,s''): probability of state s take action a
				int space[16], num = 0;
				for (int i = 0; i < 16; i++)
					// empty after action
				...
				// still has empty tile
				if (num){
					// evaluate the expectation of value after take action
					for(int i = num-1; i>=0; i--){
						board after_b;
						after_b = move->after_state();
						after_b.set(space[i], 1);
						// P(select_num)*P(select_space)*V(s'')
						second_term += estimate(after_b)*0.9/num;
						after_b.set(space[i], 2);
						second_term += estimate(after_b)*0.1/num;
					}
				}
				move->set_value(move->reward() + second_term);
	...
	// after state:
	//   V(s0') = V(s0') + alpha*(r1 + V(s1') - V(s0'))
	// state:
	//   V(s0) = V(s0) + alpha*(r0 + V(s1) - V(s0))
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		for (path.pop_back(); path.size(); path.pop_back()) {
			...
			// (r0 + V(s1)) - V(s0)
			// value already calculate at select_best_move
...
int main(int argc, const char* argv[]) {
	...
	float alpha = 0.1;
	// unsigned int compatable on 64-bit and 32-bit
	// total: number of rounds
	size_t total = 100000;
	// seed: use for random
	unsigned seed;
	// force to read the fresh timestamp
	// rdtsc: read timestamp(64-bit)
	// a: EAX=> low part(32-bit)
	// seed = a
